...
//			firstElev1->Set(-0.45);
		...
		if(secondEncoder1 > 5500) // more pos, goes up
		{
	...
	void liftAscend() {
		...
		if(secondEncoder1 < 28000) // more pos, goes up
		{
			secondElev->Set(-0.90);
		}
		else
	...
	void armUp()
	{
		int encoder = intakePivot->GetSensorCollection().GetPulseWidthPosition();
